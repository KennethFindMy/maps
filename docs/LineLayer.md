...
### Description
Defines a gradient with which to color a line feature. Can only be used with GeoJSON sources that specify `"lineMetrics": true` `<Mapbox.ShapeSource lineMetrics={true}>`.
...
Example: `lineGradient:[ 'interpolate', ['linear'], ['line-progress'], 0, 'green', 1, 'red']`
